...
function C(opts) {
  ...
  if (fs.existsSync(C.paths.settingsJSON)) {
    fs.readFile(C.paths.settingsJSON, { encoding: "utf-8" }, function(
      err,
      raw
    ) {
      if (err) {
  ...
  if (fs.existsSync(C.paths.currentChallenge)) {
    fs.readFile(C.paths.currentChallenge, { encoding: "utf-8" }, function(
      err,
      raw
    ) {
      if (err) {
...
C.prototype.validateLocalData = function() {
  ...
  fs.readFile(C.paths.settingsJSON, { encoding: "utf-8" }, function(err, data) {
    if (err)
      throw new Error(
        "Unable to read from " +
          C.paths.settingsJSON +
          ". Run `codewars setup` first."
      );
    ...
    if (!language)
      throw new Error("Language not found, run 'codewars setup' first.");
    if (!/ruby|javascript/.test(language))
      throw new Error(language + " is unsupported. Ruby and JS only.");
  ...
  if (fs.existsSync(currentChallenge)) {
    ...
    prompt.get(
      ...
      function(err, result) {
        ...
        if (/^y/.test(answer)) {
          fs.unlink(currentChallenge, function(err) {
            if (err) console.log(err);
          });
          df.resolve();
        }
...
C.prototype.fetch = function(id) {
  ...
  this.checkCurrentChallenge()
    ...
    .then(df.resolve.bind(this), df.reject.bind(this));
...
C.prototype.attempt = function(solution) {
  ...
  this.validateLocalData().then(function(args) {
    if (fs.existsSync(currentChallenge)) {
      ...
      df.reject(
        new Error("Challenge not in progress. Run 'codewars train' first.")
      );
    }
  });
